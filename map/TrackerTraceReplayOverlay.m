...
struct TrackerTraceReplayPoint{
    double x;           ///<x坐标
    double y;           ///<y坐标
    int    weight;      ///<权重
...
typedef struct TrackerTraceReplayPoint TraceReplayPoint;

@interface TrackerTraceReplayOverlay ()
{
    
    MAMultiPolyline *_multiPolyline;
    
    MAPolyline *_patchLine;
    
    MAMapPoint  _patchLinePoints[2];
    TraceReplayPoint *_origMapPoints;
...
@implementation TrackerTraceReplayOverlay
- (instancetype)init
{
...
- (BOOL)setWithCoordinates:(CLLocationCoordinate2D *)coords count:(NSInteger)count
{
    ...
    TraceReplayPoint *oldPoints = _origMapPoints;
    
    _origMapPoints = (TraceReplayPoint *)malloc(count*sizeof(TraceReplayPoint));
    ...
    TraceReplayPoint *curP1 = _origMapPoints;
...
- (BOOL)setWithPoints:(MAMapPoint *)points count:(NSInteger)count
{
    ...
    for(NSInteger i = 0 ;i<count-1;++i)
    {
        ...
        MAMapPoint p1 = MAMapPointMake(_origMapPoints[i].x, _origMapPoints[i].y);
         MAMapPoint p2 = MAMapPointMake(_origMapPoints[i+1].x, _origMapPoints[i+1].y);
        MAMetersBetweenMapPoints(p1, p2);
        _origMapPoints[i].distance = MAMetersBetweenMapPoints(p1, p2);
    }
    if(oldPoints != NULL) {
        free(oldPoints);
...
- (void)reducer_RDP:(TraceReplayPoint *)inPoints
